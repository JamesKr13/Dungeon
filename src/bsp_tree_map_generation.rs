...
impl Leaf {
    ...
    fn split(&mut self) -> Option<(Leaf, Leaf)> {
        ...
        if self.width > self.height && f32::from(self.width / self.width) >= 1.25 {
            horizon_split = false;
        } else if self.height > self.width && f32::from(self.height / self.width) >= 1.25 {
            horizon_split = true;
        }
        ...
        match horizon_split {
            true => Some((
                self.inital(self.x, self.y, self.width, split),
                self.inital(self.x, self.y + split, self.width, self.height - split),
            )),
            false => Some((
                self.inital(self.x, self.y, split, self.height),
                self.inital(self.x + split, self.y, self.width - split, self.height),
            )),
        }
    }
    fn create_rooms(&mut self) {
        ...
        if self.width >= 6 && self.height >= 6 {
            ...
            let room_pos: [i16; 2] = [
                rng.gen_range(0..self.width - roomsize[0]),
                rng.gen_range(0..self.height - roomsize[1]),
            ];
            self.room = Rectangle {
...
impl Default for BSPTree {
    fn default() -> Self {
        Self {
            leafs: Vec::new(),
            root: new_leaf(0, 0, WORLD_SIZE.0 as i16, WORLD_SIZE.1 as i16),
...
impl BSPTree {
        ...
        for leaf in &self.leafs {
            for y in 0..leaf.room.y2 {
                for x in 0..leaf.room.x2 {
                    self.level[(x + leaf.room.x1) as usize][(y + leaf.room.y1) as usize] =
                        TileType::Floor;
                }
            }
        }
    ...
    fn create_hallway(&mut self) {
        ...
        for room_index in 0..self.leafs.len() {
            let main_room = (
                self.leafs[room_index].room.x1 + (self.leafs[room_index].room.x2) / 2,
                self.leafs[room_index].room.y1 + (self.leafs[room_index].room.y2) / 2,
            );
            let mut closes_distance: f32 = f32::INFINITY;
            for other_room_index in 0..self.leafs.len() {
                if other_room_index != room_index {
                    let compare_room = (
                        self.leafs[other_room_index].room.x1
                            + (self.leafs[other_room_index].room.x2) / 2,
                        self.leafs[other_room_index].room.y1
                            + (self.leafs[other_room_index].room.y2) / 2,
                    );
                    let distance =
                        pythogoras(main_room.0 - compare_room.0, main_room.1 - compare_room.1);
                    if distance <= closes_distance
                        && main_room
                            != match linked_rooms.get(&compare_room) {
    ...
    fn create_passage(&mut self, point: (i16, i16), second_point: (i16, i16)) {
        ...
        for x_distance in *vec![point.1, second_point.1].iter().min().unwrap()
            ..=*vec![point.1, second_point.1].iter().max().unwrap() + 2
        {
            ...
            if destination_point < WORLD_SIZE.1 as i16 {
                self.level[(destination_point + 1) as usize][x_distance as usize] = TileType::Floor;
            }
        ...
        for y_distance in *vec![point.0, second_point.0].iter().min().unwrap()
            ..=*vec![point.0, second_point.0].iter().max().unwrap() + 2
        {
            self.level[y_distance as usize][destination_point as usize] = TileType::Floor;
            if destination_point < WORLD_SIZE.0 as i16 {
...
fn pythogoras(opp: i16, adj: i16) -> f32 {
    f32::from(opp.pow(2) + adj.pow(2))
}
#[must_use]
pub fn abs(value: i16) -> i16 {
    f32::from(value.pow(2)).sqrt() as i16
}
